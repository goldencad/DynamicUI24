# TS24 Dynamic UI Framework — Specification v0.9
...
Adds a **Search, Command & Context Navigation Foundation** and formalizes a
ChatGPT-inspired but application-neutral Shell interaction model for large TS24 desktop
applications.

Major additions:

1. **Global Search / Command Palette**
   - a Shell-level Global Search entry point, normally presented in the Top Shell;
   - `Ctrl+K` on Windows/Linux and `Cmd+K` on macOS as the standard launcher shortcut;
   - provider-based search through `ISearchProvider`;
   - typed results for navigation, commands, settings, records/documents/reports, and
     recent/favorite/pinned destinations;
   - permission, Company Context, localization, deterministic ranking, cancellation,
     and failure isolation.

2. **Three Search Scopes**
   - `GLOBAL_SEARCH`: cross-application discovery through registered providers;
   - `NAVIGATION_SEARCH`: filter/find within the active navigation Tree/subtree;
   - `WORKSPACE_SEARCH`: template/application-owned search within the active workspace.
   These scopes are intentionally distinct and must not be collapsed into one
   database-aware search box.

3. **Favorites, Pinned, and Recent**
   - user-controlled Favorites;
   - user-controlled Pinned destinations;
   - automatically maintained Recent destinations/actions where appropriate;
   - these are presentation/navigation preferences and never change published
     application metadata or business authorization;
   - deterministic ordering and Company/workspace scoping.

4. **Context Navigation**
   - optional Breadcrumb;
   - optional reusable right-side Context / Inspector Panel;
   - semantic `HelpContextCode`;
   - progressive disclosure of details, properties, help, activity, and future
     assistant integrations without cluttering the main workspace.

5. **Shell Composition Guidance**
   - compact Top Shell with Global Search and notification/user utilities;
   - collapsible/resizable navigation remains a shared capability;
   - main workspace remains visually dominant;
   - optional Context Panel appears only when useful;
   - progressive disclosure is preferred over permanently exposing every command.

6. **Dynamic Context / Flyout Menu Foundation**
   - reusable metadata-driven flyout/context menus for overflow buttons, dropdowns,
     split buttons, Tree/Grid/document/notification rows, and Shell utilities;
   - command, submenu, toggle, check, radio, separator, and header item kinds;
   - pointer-hover submenu behavior, full keyboard navigation, automatic edge-aware
     placement, permission/capability resolution, destructive-action semantics, and
     dynamic `IMenuProvider` extensibility;
   - one reusable `MenuDefinition` shared by dropdown/split/overflow triggers rather
     than separate menu systems.

7. **Notification Surface Integration Clarification**
   - one logical notification may render on Notification Center, Top Action Bar,
     Bottom Action Bar, Banner, Alert Card, or Toast according to metadata;
   - Top/Bottom Action Bars reuse existing Dynamic Action Bar infrastructure rather
     than introducing a second status/action framework;
   - per-surface presentation density may be Icon-only, Compact, Standard, or Detailed;
   - lifecycle, deduplication, dismissal, resolution, permission, Company Context, and
     actions remain shared across all rendered surfaces.

v0.9 is an additive evolution of the authoritative v0.8 repository specification.

Authoritative v0.8 lineage:

```text
docs/specification/DynamicUI24-Spec-v0.8.md
SHA-256: 9a3d11940e02ee1a86713dced8d88d60dee58a1321f35178eaa0bb38a9969766
```

Historical specification files remain immutable. v0.9 does not modify or replace the
v0.8 artifact; it becomes the next specification baseline only after review, commit,
push, and repository verification.
...
## 74. Shell Interaction Model — Progressive, Searchable, Contextual

DynamicUI24 desktop applications may contain hundreds of workspaces, commands, records,
reports, documents, and contextual actions. The Shell must remain understandable as the
application grows.

The v0.9 Shell interaction model adopts these principles:

1. **Main workspace remains visually dominant.**
2. **Navigation stays discoverable but may collapse when space is needed.**
3. **Search is available near global navigation, not buried inside application menus.**
4. **Frequently used destinations can be Pinned or Favorited.**
5. **Recent activity provides quick return paths without becoming authoritative history.**
6. **Secondary information is progressively disclosed through menus, panels, or context
   surfaces instead of permanently occupying the workspace.**
7. **The same navigation, permission, Company Context, localization, theme, and command
   infrastructure is reused across Tree, Search, Favorites, Recents, Notifications, and
   Guidance.**
8. **No Shell control may bypass backend/business authorization.**

A reference desktop composition is conceptually:

```text
┌──────────────────────────────────────────────────────────────────────────────┐
│ App / Ribbon            Global Search / Command Palette      🔔  Utilities │
├──────────────────────────────────────────────────────────────────────────────┤
│ Optional Top Action / Notification Surface                                  │
├───────────────┬───────────────────────────────────────┬──────────────────────┤
│ Navigation    │ Optional Breadcrumb                   │ Optional Context     │
│ Tree          ├───────────────────────────────────────┤ / Inspector Panel   │
│ + Filter      │                                       │                     │
│ Favorites     │            Active Workspace           │ Details / Help      │
│ Pinned        │                                       │ Properties / Context│
│ Recent        │                                       │                     │
├───────────────┴───────────────────────────────────────┴──────────────────────┤
│ Optional Bottom Action / Status / Notification Surface                      │
└──────────────────────────────────────────────────────────────────────────────┘
```

This diagram is behavioral guidance, not a fixed pixel layout.

The framework must not require every application to display every region
simultaneously.


## 75. Search Architecture

### 75.1 Three Distinct Search Scopes

DynamicUI24 distinguishes:
...
These scopes solve different user problems.

#### GLOBAL_SEARCH

Purpose:

> Find where to go or what action/object is available across the application.

Global Search is Shell-level and provider-driven.
...
- recent/favorite/pinned destinations;
- records/documents/reports supplied by application-specific providers.

DynamicUI24 does not directly query application databases.

#### NAVIGATION_SEARCH

Purpose:

> Find a destination inside the current navigation hierarchy.

Navigation Search filters or matches the active Tree/subtree while preserving semantic
navigation identity.

It does not become a generic record/database search.

#### WORKSPACE_SEARCH

Purpose:

> Find/filter content inside the current workspace.

Workspace Search belongs to the active template/application workspace.

Examples include a DataEntry Grid row search, Document search, Report search, or
History search.

The Shell defines integration contracts but does not prescribe the data-query
implementation.

### 75.2 Search Placement

Global Search should normally be exposed in the **Top Shell**, near global navigation
and utility controls.

Conceptual placement:

```text
App / Ribbon                 [ Search…   Ctrl/Cmd+K ]       Notifications / User
```

On constrained layouts the visible field may collapse into a search icon while the
keyboard shortcut remains available.

Global Search must not consume the entire Ribbon or replace contextual workspace
commands.

Navigation Search belongs immediately above or inside the Navigation Tree region when
enabled.

Workspace Search belongs in the active workspace's own header/action/filter region.

### 75.3 Standard Launcher Shortcut

The standard Shell launcher shortcut is:
...
Applications may expose an alternate shortcut only where platform conventions or
existing product constraints require it.

Shortcut invocation must open the same Global Search / Command Palette state as the
visible Top Shell search entry point.

### 75.4 Global Search / Command Palette

Global Search and Command Palette share one reusable Shell surface.
...
- cancellation of stale asynchronous searches;
- Company/context-safe result invalidation;
- localization and theme updates without losing the current query where safe.

The palette may show useful empty-query groups such as:
...
Suggestion behavior must remain deterministic and non-invasive. DynamicUI24 does not
perform opaque behavioral profiling.

### 75.5 Search Result Types

A generic result contract should support semantic result kinds such as:
...
Future result kinds may be registered without changing the core renderer where
possible.
...
The framework must not assume that `RECORD`, `DOCUMENT`, or `REPORT` data has one
universal schema.

### 75.6 Search Provider Boundary

Introduce a generic provider concept such as:
...
- declare provider identity and supported result kinds;
- accept a query/context;
- return framework-safe result metadata;
- honor application/backend authorization as applicable;
- support cancellation where asynchronous work is involved.

Framework responsibilities:

- orchestrate registered providers;
- isolate provider failures;
- cancel/ignore stale results;
- apply presentation-level Company/permission/capability filtering;
- merge/deduplicate results;
- rank/order deterministically;
- render and dispatch through existing navigation/command infrastructure.

The framework must not contain application-specific record search logic.

### 75.7 Search Context

Search requests should carry semantic context such as:
...
Runtime UI object references do not belong in search contracts.

### 75.8 Search Ranking

Ranking must be deterministic.

A framework implementation may consider:

- explicit provider rank/score;
- exact code/title match;
- prefix match;
- semantic result-kind priority;
- Pinned/Favorite status where explicitly configured;
- recent usage as a transparent tie-breaker;
- stable provider/result identifiers.

v0.9 does not require machine-learning ranking.

Search ranking must not silently bypass permissions or Company scope.

### 75.9 Search Deduplication

Multiple providers may expose the same logical destination.

The framework should support deterministic deduplication using semantic identity such
as:

```text
ProviderCode + ResultKind + ResultId
```

or an explicit deduplication key.

Deduplication must never merge unrelated records merely because display text matches.

### 75.10 Search Failure Safety

Safe behavior is required for:
...
- unknown WorkspaceId;
- unknown command;
- stale Company response;
- stale previous-query response;
- permission resolver failure.

One failing search provider must not make Global Search unusable.


## 76. Navigation Search / Tree Filter

The Navigation Tree supports an optional reusable Search/Filter surface.

Conceptual metadata:

```text
NavigationSearchDefinition
├─ Enabled
├─ PlaceholderKey?
├─ AutoShowAboveItemCount?
├─ SearchMode
...
1. small Trees may omit the search field;
2. large Trees may automatically expose it according to metadata/policy;
3. matching descendants remain understandable within their hierarchy;
4. parent context may remain visible even when only descendants match;
5. permission-hidden nodes never become discoverable through search;
6. disabled nodes follow the same actionability rules as the Tree;
7. Company switch re-resolves searchable nodes;
8. changing language updates searchable labels;
9. clearing the query restores the previous Tree expansion/selection state where
   practical;
10. `See more / Show less` and Navigation Search must coexist predictably.

Navigation Search must not query arbitrary application records.


## 77. Favorites, Pinned, and Recent

### 77.1 Purpose

Large enterprise applications become faster to use when users can return to common
destinations without repeatedly navigating deep Trees.

DynamicUI24 defines three distinct concepts:
...
They are user navigation/presentation preferences.

They do not modify published application definitions and do not grant authorization.

### 77.2 Favorite

A Favorite is an explicit user preference indicating:

> This destination/action is important to me.

Favorites may include framework-safe targets such as:

- Workspace;
- Tree destination;
- report definition;
- document/search destination supplied by a provider;
- registered command where appropriate.

Favorite state belongs to user preference storage/provider boundaries.

The framework may provide a star semantic icon/action.

### 77.3 Pinned

Pinned means:

> Keep this item in a persistent high-visibility quick-access region.

Pinned items are user-selected and may be ordered.
...
Pinning and Favoriting are related but not identical.

An item may be:

- Favorite but not Pinned;
- Pinned but not Favorite;
- both;
- neither.

Applications may choose to couple them through product policy, but the framework
contracts remain distinct.

### 77.4 Recent

Recent items are automatically maintained quick-return destinations.

Examples:

- recently opened workspace;
- recently opened report;
- recently visited document context;
- recently executed safe navigation command.

Recent does not mean:

- audit history;
- legal history;
- authoritative activity log;
- business transaction history.

Those remain application responsibilities.

### 77.5 Preference Identity

Quick-access entries must use stable semantic identity rather than localized text.
...
Localized labels/icons are resolved from current metadata/provider state.

### 77.6 Company Context

A Favorite/Pinned/Recent entry may be:
...
Company-scoped entries from Company A must not become actionable under Company B
without re-resolution.

Global destinations may remain visible across Company switches.

Stale or unauthorized entries fail closed.

### 77.7 Permission and Lifecycle Changes

If a user loses permission or a published destination is retired:

- the entry must not remain actionable;
- the UI may hide, disable, or mark it unavailable according to policy;
- the preference identity may remain stored so it can recover if the target becomes
  valid later;
- no authorization may be inferred from Favorite/Pinned status.

### 77.8 Ordering

Pinned items use explicit user order where supported.

Favorites use stable configured/user order or deterministic label/code ordering.

Recent uses recency with a bounded in-memory/persisted preference policy supplied by the
consumer.

DynamicUI24 must not allow Recent lists to grow unbounded.

### 77.9 Shell Presentation

Favorites/Pinned/Recent may appear in:

- Navigation Pane;
- Global Search empty-query state;
- Application Menu contribution where appropriate;
- a compact Quick Access region.

The framework should avoid showing the same entry redundantly in every region by
default.

### 77.10 User Operations

Framework-safe operations may include:
...
These are presentation-preference operations, not business commands.

They must not mutate application records.


## 78. Breadcrumb and Location Context
...
They are optional because shallow applications/workspaces may not need them.

Conceptual examples:

```text
Setup > Master Catalogs > Employees
Documents > Contracts > 2026
```

### 78.2 Breadcrumb Definition
...
Breadcrumb navigation must reuse existing navigation infrastructure.

### 78.3 Responsive Behavior

When horizontal space is insufficient:

- middle ancestors may collapse into an overflow menu;
- current destination remains visible;
- breadcrumb must not force the main workspace to unreasonable minimum width.

No application should depend on breadcrumb text as business state.

### 78.4 Context Synchronization

Breadcrumb state is resolved from the same navigation/workspace context used by:
...
No parallel navigation state should be created.


## 79. Context / Inspector Panel

### 79.1 Purpose

The main workspace should not be permanently overloaded with secondary details.

DynamicUI24 therefore supports an optional reusable **Context / Inspector Panel**,
normally on the right side.

Typical non-domain-specific uses:
...
- related information;
- help;
- activity/comments supplied by an application provider;
- notification detail;
- future assistant/help integration.

### 79.2 Layout Contract
...
The Context Panel is:

- optional;
- collapsible;
- resizable;
- bounded by minimum/maximum width;
- independently scrollable where necessary;
- able to preserve main-workspace state when opened/closed/resized.

Opening the Context Panel must not recreate the active workspace merely to change
layout.

### 79.3 Context Panel Definition
...
### 79.4 Context Content Provider

The framework may expose a provider abstraction such as:

```text
IContextPanelProvider
```

Providers supply framework-safe contextual presentation models.

DynamicUI24 does not require all applications to use one universal property/details
schema.

### 79.5 Selection Context

Context Panel providers may receive semantic state such as:
...
Runtime UI object references must not be persisted in metadata.

### 79.6 Failure and Stale-State Safety

Unknown provider, provider exception, stale Company result, stale selection result, or
permission failure must not crash or contaminate the Shell.

Selection/context generation protection should follow the same stale-response principles
used elsewhere in DynamicUI24.


## 80. Contextual Help and HelpContextCode

### 80.1 Help Context

Workspaces, sections, fields, commands, and templates may expose a semantic:
...
> Allow a help/documentation/assistant provider to understand what the user is
> currently looking at without coupling the UI framework to a specific help engine.

### 80.2 Scope

`HelpContextCode` may be associated with:
...
### 80.3 Help Provider Boundary

DynamicUI24 may expose a future provider boundary for contextual help.
...
A consumer may map `HelpContextCode` to local documentation, web help, or an approved
assistant service.

### 80.4 Privacy

Contextual help integration must not automatically send application data outside the
application.

Any future assistant integration must explicitly define the data boundary separately.

v0.9 only defines semantic context hooks.


## 81. Notification Presentation Surfaces — v0.9 Clarification

v0.8 introduced Dynamic Notification & Guidance.

v0.9 clarifies that **one logical notification** may be presented on multiple existing
Shell/workspace surfaces without creating multiple lifecycle instances.

### 81.1 Presentation Targets

Generic presentation targets may include:
...
A notification may resolve to one or multiple targets.

### 81.2 Reuse Existing Action Bars

`TOP_ACTION_BAR` and `BOTTOM_ACTION_BAR` must reuse the Dynamic Action Bar
infrastructure introduced earlier.

Do not create a separate notification status/action-bar subsystem.

Notification contributions use the same:
...
### 81.3 Per-Surface Presentation Density

A notification surface definition may choose presentation density such as:
...
Conceptual per-surface metadata:

```text
NotificationSurfaceDefinition
├─ Surface
├─ DisplayMode
├─ ShowIcon
├─ ShowTitle
...
Exact implementation naming may vary while preserving semantics.

### 81.4 Shared Logical State

Rendering one notification simultaneously in:
...
must still represent **one logical notification**.

The following state is shared across surfaces:
...
A surface renderer must not fork notification business/runtime state.

### 81.5 Application Update UX Example

An application update is a valid example of notification/guidance presentation, but the
updater engine does not belong to DynamicUI24.

Conceptually:

```text
UPDATE_AVAILABLE
→ Notification Center: detailed/reviewable
→ Top Action Bar: "Version 4.2.1 ready" [Update & Restart]
→ Bottom Action Bar: compact "Update ready"
→ Alert Card: auto-show once when appropriate
```

All surfaces dispatch the same registered application command.

DynamicUI24 does not:
...
Those responsibilities belong to application/shared update infrastructure.

This example is normative only for architecture separation, not for product-specific
wording or command names.



## 82. Dynamic Context / Flyout Menu Foundation

DynamicUI24 formalizes a reusable context/flyout menu system for compact, progressive
disclosure of secondary actions.
...
It must reuse existing command, navigation, permission/capability, icon, localization,
theme, and design-system infrastructure.

It must not create a parallel business-command framework.

### 82.1 Purpose

Context/Flyout menus allow enterprise applications to keep the primary workspace visually
clean while still exposing secondary and contextual operations.
...
The menu structure is metadata-driven.

Consumer applications should not normally hard-code menu hierarchy directly into shared
Shell/control XAML.

### 82.2 MenuDefinition

Conceptual reusable contract:
...
`MenuCode` is a stable semantic identifier.

### 82.3 MenuItemDefinition

Conceptual item contract:
...
- `COMMAND`: executes one registered command/navigation action;
- `SUBMENU`: opens child items without executing an action;
- `TOGGLE`: represents a reversible presentation/action state;
- `CHECK`: represents a checked/unchecked option;
- `RADIO`: selects one option inside a semantic group;
- `SEPARATOR`: visually groups related actions;
- `HEADER`: non-actionable contextual group label.

Unknown item kinds fail safely.

### 82.5 Submenu Depth

Submenus must support the practical interaction pattern used by modern desktop
applications.

Default supported hierarchy:
...
A framework implementation may support deeper nesting internally, but published
application metadata SHOULD normally remain at no more than two actionable submenu
levels unless a later specification explicitly extends this recommendation.

Deep menu trees should be replaced by Search, dedicated workspace navigation, or a
Context Panel when possible.

### 82.6 Opening and Placement

A Flyout Menu may be opened by:

- button click;
- chevron segment of a Split Button;
- overflow (`...`) action;
- context-menu invocation;
- keyboard invocation where supported.

Placement policy should support semantic positions such as:
...
`AUTO` is preferred for reusable controls.

The renderer must reposition/flip the flyout when the requested direction would overflow
the usable application/window area.

The menu must not become inaccessible merely because the trigger is near a window edge.

### 82.7 Hover and Submenu Behavior

For pointer interaction:

1. hovering a normal actionable item highlights the whole row;
2. hovering an item with children may open its submenu after an appropriate short delay;
3. moving between a parent item and its child submenu should not cause accidental closure;
4. submenu presentation should remain stable near window edges;
5. disabled items remain visible where policy requires but do not appear actionable;
6. hover must not change item geometry.

Exact hover timing is a design-system/runtime choice and is not fixed by the
specification.

### 82.8 Keyboard Interaction

Context/Flyout menus must support keyboard-first operation.
...
Enter       → activate selected actionable item
Space       → toggle/check where appropriate
Esc         → close current submenu/menu
Home / End  → first/last actionable item where supported
```

Focus must be visible.

Headers and separators are skipped by normal actionable keyboard traversal.

### 82.9 Click / Close Behavior

Default behavior:

- clicking an actionable command executes the action and closes the menu;
- clicking outside the menu closes it;
- pressing Esc closes it;
- opening a sibling submenu closes the previous sibling submenu.

A menu item may explicitly request a different close policy for safe presentation
operations such as multi-toggle settings.

Conceptual close policies:
...
Business operations should normally use `CLOSE_ON_INVOKE`.

### 82.10 Dynamic Context Resolution

Menu content may resolve against runtime semantic context such as:
...
Selection/context changes may update:
...
Runtime UI object references must not be stored in menu metadata.

### 82.11 Permission and Capability

Each menu and item may declare existing:
...
Behavior remains consistent with DynamicUI24:
...
Permission resolver failure must fail closed.

A hidden privileged action must not remain discoverable through child submenu text,
shortcut display, or search metadata unintentionally.

### 82.12 Command and Navigation Reuse

Actionable menu items must dispatch through existing shared infrastructure:
...
NavigationTarget
   ↓
Existing Command / Navigation Service
```

Do not:

- execute arbitrary reflection targets;
- store executable scripts in menu metadata;
- instantiate application workspaces directly;
- create a separate context-menu command bus.

### 82.13 Check, Toggle, and Radio State

`CHECK`, `TOGGLE`, and `RADIO` items represent resolved presentation/action state.
...
The menu renderer must not invent authoritative business state.

`RADIO` items use a stable `RadioGroupCode` so only one resolved item in that group is
selected according to the supplying state/provider.

### 82.14 Destructive Actions

A menu item may be marked:

```text
IsDestructive = true
```

This affects semantic presentation only.

A destructive marker does not replace application confirmation, permission, validation,
transaction, or backend authorization.

Shared UI should visually distinguish destructive actions using semantic design tokens,
not hard-coded product colors.

### 82.15 Shortcuts

Menu items may display a shortcut hint:

```text
ShortcutDisplay
```

The displayed shortcut must reflect an actually registered shortcut where applicable.

Displaying shortcut text does not itself register or execute the shortcut.

### 82.16 Localization and Theme

All labels use localization resources or framework-safe resolved text.

Technical identifiers remain untranslated.

Menus must update correctly during runtime:
...
without changing semantic menu identity.
...
- keyboard reachable;
- focus visible;
- screen-reader semantics for item role/state;
- checked/radio/toggle state exposed semantically;
- submenu ownership/expanded state exposed where platform accessibility permits;
- destructive meaning must not rely on color alone;
- icon-only menu triggers require accessible labels/tooltips.

### 82.18 Context Menu Trigger

Reusable row/item surfaces may expose a semantic context-menu trigger.
...
The trigger should normally remain visually unobtrusive until hover/focus/context makes
it useful, unless product design explicitly requires permanent visibility.

This supports progressive disclosure while keeping dense enterprise UI clean.

### 82.19 Dynamic Menu Providers

For menus whose contents depend on current runtime state, DynamicUI24 may expose a
provider abstraction such as:
...
- return framework-safe menu definitions/items for the supplied semantic context;
- honor backend/application authorization;
- support cancellation if asynchronous work is required.

Framework responsibilities:

- failure isolation;
- permission/capability presentation;
- stale-context protection;
- deterministic ordering;
- rendering;
- command/navigation dispatch.

Simple static menus do not require a provider.

### 82.20 Failure Safety

The menu system must fail safely for:
...
- unknown IconKey;
- unknown command;
- unknown NavigationTarget;
- invalid submenu hierarchy;
- provider exception;
- stale Company/selection context;
- permission resolver failure;
- malformed check/radio state.

A malformed contextual menu must not crash the Shell or parent workspace.

### 82.21 Reuse Across Action Controls

Existing action variants remain the trigger foundation:
...
`DROPDOWN_BUTTON` and `SPLIT_BUTTON` should consume `MenuDefinition` rather than invent a
second menu metadata shape.

An overflow Icon Button (`...`) may also consume the same `MenuDefinition`.

### 82.22 Progressive Disclosure Rule

Context/Flyout menus should contain secondary or contextual actions.

Frequently used primary actions should normally remain directly discoverable through:
...
Do not hide every common operation inside `...`.

Conversely, do not permanently expose every low-frequency command merely because screen
space exists.

The framework should allow application designers to balance direct discoverability and
progressive disclosure through metadata.



## 83. Shell Quick Access and Progressive Disclosure Rules

### 83.1 Quick Access

A Shell may expose a compact Quick Access region containing a bounded subset of:

- Pinned destinations;
- Favorites;
- Recent destinations;
- high-value global commands.

User-customized quick access must not become a second independent navigation system.

Every destination still resolves through the same shared navigation/command contracts.

### 83.2 Avoid Permanent Visual Overload
...
over permanently placing every possible command on:
...
### 83.3 Progressive Disclosure

Secondary actions/detail should appear when:

- context makes them relevant;
- the user explicitly opens an overflow/menu/panel;
- a provider emits actionable attention state;
- search/palette surfaces them.

Primary work should remain obvious without opening multiple panels.

### 83.4 Resizable and Collapsible Regions

The Shell may combine:

- collapsible/resizable left Navigation Pane;
- main Workspace;
- collapsible/resizable right Context Panel.

The main workspace must retain a usable minimum area.

Per-user pane width/open-state preferences are presentation preferences, not published
business metadata.

### 83.5 Keyboard Navigation

At minimum, Shell interaction architecture should permit:

- `Ctrl/Cmd+K` → Global Search / Command Palette;
- keyboard traversal of search results;
- keyboard access to Notification Center;
- keyboard access to Pinned/Favorite actions;
- visible focus on interactive Shell controls.

Additional shortcuts remain application/platform policy.

### 83.6 Mobile/Small Window Interpretation

DynamicUI24 is desktop-first, but Shell contracts should degrade gracefully at narrow
window widths.

Possible behavior includes:

- collapsing Navigation Pane;
- collapsing the Global Search field to an icon;
- hiding optional Breadcrumb ancestors;
- closing/collapsing Context Panel;
- moving non-primary utilities into overflow.

The specification does not require a phone/mobile application shell.


## 84. Search, Quick Access, and Context Security Rules

Search and quick access are convenience features, not authorization mechanisms.

Required rules:

1. permission-hidden destinations must not leak through Search, Recent, Favorite, Pinned,
   Breadcrumb, or Context Panel;
2. disabled/read-only semantics must remain consistent with existing capability
   presentation;
3. backend/business authorization remains authoritative;
4. Company-scoped results/preferences must re-resolve on Company switch;
5. stale asynchronous results from a previous Company/query/selection must not become
   actionable;
6. provider exceptions and raw sensitive error text must not surface directly to users;
7. search providers must return the minimum presentation data needed for discovery;
8. Future AI/help providers must not receive data implicitly merely because
   `HelpContextCode` exists.


## 85. v0.9 Extensibility and Local-AI Maintainability

A future application should be able to extend search/context UX primarily by
registration rather than Shell modification.

Expected extension pattern:

```text
Register Workspace / Tree metadata
        +
Register Search Provider(s)
        +
Register Context Provider(s) if needed
        +
Publish localization / semantic icons
        +
User builds Favorites / Pins / Recents
        ↓
Shared DynamicUI24 Shell
```

Adding an application Search Provider must normally require no modification to the
Global Search renderer.

Adding a Context Panel provider must normally require no modification to Shell layout
logic.

Focused module documentation should identify:
...
This preserves the least-context maintenance model introduced in v0.8.



## 86. Definition of Done for Specification v0.9

Specification v0.9 is ready for implementation/adoption when these are accepted:
...
- Shell progressive-disclosure interaction model confirmed;
- Dynamic Context/Flyout Menu foundation confirmed;
- reusable `MenuDefinition` / `MenuItemDefinition` contracts confirmed;
- COMMAND/SUBMENU/TOGGLE/CHECK/RADIO/SEPARATOR/HEADER item kinds confirmed;
- edge-aware flyout/submenu placement confirmed;
- hover and full keyboard submenu interaction confirmed;
- permission/capability/context resolution confirmed;
- destructive-action and shortcut-display semantics confirmed;
- dynamic `IMenuProvider` extension boundary confirmed;
- dropdown/split/overflow controls reuse one menu contract confirmed;
- Global Search / Command Palette contract confirmed;
- standard `Ctrl+K` / `Cmd+K` launcher confirmed;
- `GLOBAL_SEARCH`, `NAVIGATION_SEARCH`, and `WORKSPACE_SEARCH` separation confirmed;
- `ISearchProvider` boundary and failure isolation confirmed;
- typed SearchResult contract and deterministic ranking/deduplication confirmed;
- Search permission/Company/stale-response protections confirmed;
- Navigation Tree Search/Filter contract confirmed;
- Favorite semantics confirmed;
- Pinned quick-access semantics confirmed;
- Recent quick-return semantics and bounded retention confirmed;
- quick-access preference identity and Company scope confirmed;
- optional Breadcrumb contract confirmed;
- optional resizable/collapsible Context / Inspector Panel confirmed;
- contextual provider boundary confirmed;
- semantic `HelpContextCode` confirmed;
- Notification presentation on Top/Bottom Action Bars confirmed;
- notification per-surface density contract confirmed;
- one logical notification across multiple surfaces confirmed;
- application-update example remains separated from updater implementation confirmed;
- responsive/collapsible Shell behavior confirmed;
- Search/Quick Access/Context security rules confirmed;
- extension through providers/metadata without core Shell modification confirmed;
