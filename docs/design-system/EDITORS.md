...
Text and memo use native OS input. Numeric/date/choice use native Avalonia controls. Floating labels are metadata-supported but not forced where they would destabilize an active native input session. Error state must be perceivable through text/accessibility, not color alone. P1 may mask or omit value, helper, tooltip, clipboard and accessibility output.
